...
export default function MahjongPage() {
  ...
  useEffect(()=>{if(!ws.connected)return;const u:(()=>void)[]=[];
    ...
    u.push(ws.onMessage('mahjong_game_state',(p:ClientState)=>{setGs(p);setMySeat(p.seat??mySeat);if(p.currentPlayer!==p.seat)setActs(null);setSel(-1);}));
    u.push(ws.onMessage('mahjong_discarded',(p:any)=>{if(p.state)setGs(p.state);}));
    ...
    u.push(ws.onMessage('mahjong_game_ended',()=>{setGs(null);setSel(-1);setActs(null);setPhase('playing');}));
  ...
  const discard=useCallback(()=>{if(sel<0)return;send('mahjong_discard',{tileIndex:sel});setSel(-1);},[sel,send]);
  ...
  const Melds=({seat,side,rot}:{seat:number;side?:boolean;rot?:number})=>{const m=s?.melds?.[seat];if(!m?.length)return null;return <div className={side?'mj-side-melds':''} style={!side?{display:'flex'}:{}}>{m.map((g:MeldData,i:number)=><div key={i} className="mj-meld-group" style={{display:'flex',flexDirection:side?'column':'row'}}>{g.type==='concealed_gang'?<><TileView suit={g.tiles[0].suit} value={g.tiles[0].value} small rotation={rot}/><TileView suit="wan" value={1} hidden small rotation={rot}/><TileView suit="wan" value={1} hidden small rotation={rot}/><TileView suit={g.tiles[3].suit} value={g.tiles[3].value} small rotation={rot}/></>:g.tiles.map((t,j)=><TileView key={j} suit={t.suit} value={t.value} small rotation={rot}/>)}</div>)}</div>;};
  const Discards=({seat}:{seat:number})=>{const d=s?.discards?.[seat]||[];if(!d.length)return <div style={{minWidth:40,minHeight:30}}/>;const last=s?.lastDiscardPlayer===seat&&s?.lastDiscard;return <div style={{display:'flex',flexWrap:'wrap',gap:1,maxWidth:180,justifyContent:'center'}}>{d.slice(-18).map((t,i)=>{const isLast=last&&i===d.slice(-18).length-1&&t.suit===s!.lastDiscard!.suit&&t.value===s!.lastDiscard!.value;return <TileView key={i} suit={t.suit} value={t.value} small className={isLast?'mj-discard-last':undefined}/>;})}</div>;}
  ...
  const Position=({seat,h}:{seat:number;h:boolean})=>{
    ...
    if(!h) {const isRight=seat===rightS; const cols=[<div key="c" data-alias={c3} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'#ecf0f1',fontSize:14,fontWeight:600,writingMode:'vertical-rl'}}>{nm}</div>,<div key="b" data-alias={c2} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>{hasM?md:null}</div>,<div key="a" data-alias={c1} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>{hd}</div>]; return <>{isRight?cols.reverse():cols}</>;}
    return <><div data-alias={r3} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'#ecf0f1',fontSize:14,fontWeight:600}}>{nm}</div><div data-alias={r2} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>{hasM?md:null}</div><div data-alias={r1} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>{hd}</div></>;
  };
  ...
  return (<>
    ...
    <aside className="room-sidebar">
      ...
      <div className="player-list"><h3>玩家 ({humanCount}/4)</h3>
        {[0,1,2,3].map(seat=>{const p=players.find(x=>x.seat===seat);const im=p&&p.username===uname;const canStand=im&&!isOwner;const click=()=>{if(canStand)stand();else if(!im&&!s)sit(seat);};
...
<div className={`mj-flow-strip-v ${s?.currentPlayer===rightS?'active':''}`} style={{position:'absolute',right:0,top:'20%',bottom:'20%',width:3}}/>
          {s&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,width:'90%',marginTop:20}}><Discards seat={topS}/><Discards seat={rightS}/><Discards seat={leftS}/><Discards seat={mySeat||0}/></div>}
        </div>
      ...
      <div style={{width:'55%',flex:1,display:'flex',flexDirection:'column'}}>
        {!s?<>
          <div onClick={gn(mySeat||0)===uname&&!isOwner?stand:undefined} style={{cursor:gn(mySeat||0)===uname&&!isOwner?'pointer':'default',flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'#ecf0f1',fontSize:14,fontWeight:600}}>{gn(mySeat||0)?<>{gn(mySeat||0)===uname&&isOwner?'👑 ':'👨 '}{SEAT_NAMES[(mySeat||0)]} {gn(mySeat||0)}</>:<Empty seat={mySeat||0}/>}</div>
          <div style={{flex:1}}/><div style={{flex:1}}/>
        </>:<>
          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}><Melds seat={mySeat||0}/></div>
    ...
    <aside className="room-chat"><h3>聊天</h3><div className="chat-messages">{chatLog.map((c,i)=><div key={i}>{c.isSystem?<div className="chat-msg system"><span>{c.text}</span></div>:<div className={'chat-msg '+(c.username===uname?'me':'other')}>{c.username}: {c.text}</div>}</div>)}<div ref={cr}/></div><div className="chat-input"><input placeholder="输入消息..." value={chatMsg} onChange={e=>setChatMsg(e.target.value)} onKeyDown={e=>e.key==='Enter'&&chat()}/><button onClick={chat}>发送</button></div></aside>
    </div></>
  );
}
